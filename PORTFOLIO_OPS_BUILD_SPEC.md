...
- Free-tier hosting and database (cost is only the pay-per-use Anthropic API usage)
- Should run locally for testing before every deploy

## 3. Architecture

```
Browser (React/Vite)  ->  Vercel serverless functions (/api/*)  ->  Anthropic API (server-side key)
                                        |
...
- **Backend**: Vercel serverless functions under `/api`. Each one is a small Node.js
  handler. The Anthropic API key lives only in Vercel's environment variables, read via
...
- **Storage**: Vercel KV (a hosted Redis). Four keys: `holdings`, `events`, `briefs`,
  ...
  since this is a two-person private tool (add auth later if the URL will ever be public).
...
- [ ] Anthropic API key from console.anthropic.com, with billing enabled
...
4. Open .env.local and confirm it has KV_REST_API_URL, KV_REST_API_TOKEN, and remind me to
   manually add ANTHROPIC_API_KEY=<my key> to that same file for local testing.
...
1. Confirm the ANTHROPIC_API_KEY environment variable is set in the Vercel project
...
|---|---|---|
| `ANTHROPIC_API_KEY` | Vercel project settings + local `.env.local` | Server-side calls to the Anthropic API from `/api` routes |
...
# Get this from https://console.anthropic.com (Settings -> API Keys). Requires billing to be set up.
